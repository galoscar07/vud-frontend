...
function ClinicPage({props}) {
  ...
  const mapServerRespToFront = (serverClinic) => {
    ...
    return {
      ...
      typeOfClinic: serverClinic?.medical_unit_types?.map((mut) => {
        return mut.label
      }).join(", "),
      ...
      contact: [
        {
          type: JSON.parse(serverClinic.primary_phone).label || 'Telefon',
          value: JSON.parse(serverClinic.primary_phone).value,
          icon: "phone"
        },
        serverClinic.secondary_phone && JSON.parse(serverClinic.secondary_phone).map((el) => {
          return {type: el[0], value: el[1], icon: "phone"}
        }),
        serverClinic.secondary_email && {
          ...serverClinic?.secondary_email?.split('|').map((sc) => {
            return {type: "email", value: sc, icon: "email"}
          })
        },
      ...
      clinics: serverClinic?.collaborator_clinic,
          ...
          : <React.Fragment>
            ...
            <div className="grid">
              ...
              <div className="info-right-container">
                ...
                <div className="col">
                  <Dropdown selected={selectedSpecialties} options={specialities} title={"Specialitati"}
                            onSelect={handleSubmitSpecialties}/>
                </div>
                <div className="col-2">
                  <Dropdown selected={selectedDegrees} options={academicDegreesDropDown} title={"Grade medicale"}
                            onSelect={handleSubmitDegrees}/>
                  <Dropdown selected={selectedCompetences} options={competences} title={"Competente medicale"}
                            onSelect={handleSubmitCompetences}/>
                </div>
            ...
            {!review.isReviewSubmitted && <div className="add-review-wrapper">
              {!isReviewFormDisplayed
                ?
                <button onClick={() => setIsReviewFormDisplayed(true)} className={`button border-button round`}>Adauga
                  recenzie</button>
                :
                <React.Fragment>
                  ...
                  <form onSubmit={handleAddReview} autoComplete="off" className="add-review-form">
                    ...
                    <textarea maxLength={review.maxLength} rows="6" className="full-width" type="comment" name="comment"
                      ...
                      <div className="stars-container">
                        {Array(5).fill(1).map((el, i) =>
                          <span onClick={() => setReview({...review, rating: {value: i + 1}})} key={i}>
                                                        <img
                                                          src={i >= review.rating.value ? "/images/star_empty.svg" : "/images/star_full.svg"}/>
                                                    </span>)}
                      </div>
                    </div>
